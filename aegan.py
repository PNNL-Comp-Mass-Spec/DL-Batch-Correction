...
class SupervisedAEGANModule(nn.Module):
    def __init__(self, n_features, n_batches, n_latent=100,
        ...
        latent_encoder_dims       = [n_features, 1000, 1000, n_latent]
        batch_encoder_dims       = [n_features, 1000, 1000, n_batches]
        decoder_dims       = [n_latent+n_batches, 1000, 1000, n_features]
        discriminator_dims = [n_latent, 100, 100, n_batches]
        
        self.latent_encoder       = self.multi_layer_perceptron(latent_encoder_dims)
        self.batch_encoder = nn.Sequential(self.multi_layer_perceptron(batch_encoder_dims),
                                           nn.Softmax(dim = 1))
        self.decoder       = self.multi_layer_perceptron(decoder_dims)
        self.discriminator = nn.Sequential(self.multi_layer_perceptron(discriminator_dims),
                                           nn.Softmax(dim = 1))
    ...
    def forward(self, x, y):
        y_pred = self.batch_encoder(x)
        
        z = self.latent_encoder(x)
        y_latent_pred = self.discriminator(z)
        
        y = self.to_one_hot(y)
        z_plus_y = torch.column_stack((z, y))
        x_pred = self.decoder(z_plus_y)
        
        return y_pred, y_latent_pred, x_pred
...
class SupervisedAEGAN():
    ...
    def train(self, train_data, device,
        ...
        def loss_fn(x, y, x_pred, y_pred, y_latent_pred):
            rec_loss  = nn.MSELoss()(x_pred, x)
            sup_loss  = nn.CrossEntropyLoss()(y_pred, y)
            disc_loss = nn.CrossEntropyLoss()(y_latent_pred, y)
            loss = rec_loss + supervised_weight*sup_loss - regularization * disc_loss
            metrics = {'rec_loss'  : rec_loss,
                       'sup_loss'  : sup_loss,
                       'disc_loss' : disc_loss,
                       'loss'      : loss}
        ...
        for epoch in t:
            ...
            for x, y in trainloader:
                ...
                y_pred, y_latent_pred, x_pred = self.model.forward(x, y)
                
                metrics = loss_fn(x, y, x_pred, y_pred, y_latent_pred)
            ...
            if verbose:
                t.set_description('RecLoss: {:.4f} | SupLoss: {:.4f} | DiscLoss: {:.4f}'.format(history[-1][0], history[-1][1], history[-1][2]))
